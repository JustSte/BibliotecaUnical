...
public class ReservationController {
    ...
    public ResponseEntity<String> confirmReservation(@PathVariable Long id, @AuthenticationPrincipal Jwt jwt)
    {
        String userId = jwt.getClaimAsString("sub");
        if(userId == null || userId.isBlank())
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body("User ID is missing in token");
    ...
    @PreAuthorize("hasAnyRole('USER' , 'STAFF', 'ADMIN')")
    @GetMapping("/active/resource/{resourceType}")
    @Operation(summary = "Returns the active reservation of the resource resourceType for the current user")
    public ResponseEntity<ReservationDTO> getActiveReservationForUser( @PathVariable String resourceType, @AuthenticationPrincipal Jwt jwt)
    {
        String userId = jwt.getClaimAsString("sub");
        return new ResponseEntity<>(reservationService.getActiveReservationForUser(userId, resourceType), HttpStatus.OK);
    }
    ...
    public ResponseEntity<ReservationDTO> getReservationByUserId(@PathVariable String id)
    {
        return new ResponseEntity<ReservationDTO>(reservationService.getReservationByUserId(id), HttpStatus.OK);
    }
